...
```text
┌──────────────── Android App（MaaFwApp fork 减法整理，AGPL）────────────────┐
│  主 Activity: 全屏 WebView ──► http://127.0.0.1:22267（ALAS WebUI 原版）    │
...
- 植入 ALAS 官方 master（Gitee 镜像，**钉 commit**）+ fullcn 同款国内镜像 deploy 配置；热更新只拉源码不动依赖（漂移→提示重下整包，稀有事件）。
...
- **DoD**：Termux/侧载环境手动解压，离线单命令拉起 ALAS WebUI；版本号与 BUILD_MANIFEST 可被 App 读取。
...
3. 前台服务 ProcessBuilder 拉起 proot → 自愈清锁 → Gitee 热更新（断网超时降级不阻塞）→ wrapper.py → ALAS。
...
3. 日志桥接：wrapper 读 ALAS 调度器日志推悬浮窗半透明日志板。
4. shizuku-m 未安装/未激活/与官方版冲突的引导页（README + 应用内图文，含"先卸载官方 Shizuku"提示）。
5. 分工：高频操作走悬浮窗，完整配置走 WebView。
- **DoD**：打开 App 到进控制台全程无感；游戏（虚拟屏）上点悬浮窗"开始"，ALAS 立即控制游戏，日志上悬浮窗。
...
4. 最低支持配置定档：桥方案有 m0 数据兜底（p50 0.109s）；若 Spike E 成功则实测 adb screencap/scrcpy 耗时，对照 ALAS 官方 >1s 不可用线。
...
| ALAS 内部 API 变动打破 wrapper | 🟡 | 🟡 | 面最小化 + 配置/进程组兜底 |
...
- m0 归档核查：否决 proot-distro 的三条理由（`m0-archive/docs/maaal-consensus.md:27,45,46`）在 v3 架构下全部化解（phantom 已可关、依赖现代化补丁已有、"APK 内 proot"由 Spike A 直接验证）；OCR 死链记录（`:43`）；桥性能数据（`:117`）。
...
- fullcn：官方 Releases 大陆变体 `AlasApp_0.4.10_fullcn.7z`（github.com/LmeSzinc/AzurLaneAutoScript/releases）。
