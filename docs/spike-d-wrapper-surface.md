# Spike D 探查报告：ALAS 进程管理 import 面（2026-09-15）

> 本文是路线图 v3 阶段〇 Spike D 的交付物，供 wrapper.py（rootfs 内）设计使用。探查方式为只读源码分析。

已完成探查（只读，未改动 `.tmp/alas` 任何文件）。**基线**：`.tmp/alas` = LmeSzinc/AzurLaneAutoScript `d816310324bf686dff3252b37efbb997d9a328a4`（2026-08-18，master，partial clone `blob:none`，工作树干净）。以下所有 `文件:行号` 均相对 `D:\VSCodeCache\maa-alas\.tmp\alas`。
...
- Windows 图形安装器入口是 `deploy/launcher/Alas.bat:20`（`python -m deploy.installer`），与 gui.py 无关。
- `module/webui/__init__.py` 只做一件事：把 `deploy.logger.logger` 指向 ALAS logger（第 1-5 行）。
...
| 成员 | 位置 | 语义 |
|---|---|---|
| `ProcessManager.__init__(config_name="alas")` | 28-36 | 每个实例一个对象，日志队列 `State.manager.Queue()` |
| `start(func=None, ev=None)` | 38-52 | `multiprocessing.Process(target=ProcessManager.run_process, args=(config_name, func, queue, ev))` **进程内 fork/spawn，不是 subprocess、没有命令行**；`func=None` 时用 `get_config_mod(name)`（`module/submodule/utils.py:79-89`，普通实例恒为 `'alas'`） |
...
| `restart_processes(instances, ev)` | 193-233 | 还会读 `./config/reloadalas`（218-223）补实例，启动后删除该文件（229-232） |
| `run_process(config_name, func, q, e)` | 133-183 | 子进程真正入口 |

**运行体分派**（process_manager.py:158-181）：
- `func == "alas"` → `AzurLaneAutoScript(config_name).loop()` → **调度器进程**（alas.py:549-618）。
...
- `func in get_available_mod()`（maa/fpy 子模块）→ `load_mod(func).loop(config_name)`。
- 结束统一打 `[{name}] exited. Reason: Finish`（181）。

**与 WebUI 之间的控制通道只有两条**，没有 HTTP/CLI 给调度器：
1. 共享 `manager.Event()`：`updater.event`（app.py:1443）→ `AzurLaneAutoScript.stop_event`（process_manager.py:158、164-166）→ `loop()` 检查点（alas.py:555-559）与 `wait_until`（alas.py:481-485）；置位即"优雅停"（日志 `Reason: Update`，alas.py:484）。
2. 配置文件 mtime 热重载（`config/watcher.py:23-33` ← alas.py:489）。
   按钮绑定见 app.py:453-461（总开关，`onclick_on=self.alas.stop()` / `onclick_off=self.alas.start(None, updater.event)`）与 app.py:685-689（单任务）。
...
- 导入 `module.logger` 时就会 `os.chdir(<ALAS 根>)`（logger.py:165），并在 import 期先建一份 `{date}_{sys.argv[0] 基名}.txt`（logger.py:168、337）。
- 进程启动时**只调用一次** `set_file_logger`（调度器在 `loop()` 里还会再调一次，alas.py:550；WebUI 子进程在 process_manager.py:145 调），文件以 append 打开、句柄不重开 → **日期不随午夜滚动**，wrapper tail 时路径固定为"启动日"。
- 错误快照另存 `./log/error/{time_ms}/`（alas.py:135-163）。
...
**wrapper 最省方案**：`tail -F ./log/<启动日>_<config_name>.txt`（read 侧只需处理"文件在进程启动后出现"与"整天不换名"两种情况）；若 wrapper 改为 in-process 跑 ALAS，可直接 `logger.set_func_logger(my_cb)` 拿到结构化 renderable，省掉 tail。
...
**"写配置 → 无头启动调度器"完全可行**，官方自己就有（`python alas.py`，见 Q5）：
```
cwd 必须是 ALAS 根（所有路径都是 './...' 相对路径）
...
2) 起进程：python -c "from alas import AzurLaneAutoScript as A; A('<name>').loop()"  （或等价 runner）
```
需要触碰的内部函数（若 1 步想复用 ALAS 自身的校验/补全）：
...
- **无锁文件机制**（`deploy/atomic.py` 全文无 lock），但 ALAS `save()` 每次都写整份 `data`（config.py:271-274）→ **运行中外部改配置可能被 ALAS 的下一次 save 覆盖**；改配置最稳的时机是停着改，或只改 `NextRun` 并接受竞态。
- 运行时热生效：`ConfigWatcher` mtime（watcher.py:23-33）只在 `wait_until` 检查点命中（alas.py:489）。
...
| 入口 | 位置 | 作用 |
|---|---|---|
| `alas.py` | alas.py:621-623 | `AzurLaneAutoScript().loop()` —— **官方无头调度器跑法**（config_name 固定 `'alas'`） |
| `gui.py` | gui.py:1-109 | WebUI 启动器（uvicorn + reload 循环） |
| `module/webui/app.py` | app.py:1475-1546 | ASGI app 工厂（pywebio 页面 + 生命周期） |
| `deploy/installer.py` | installer.py:13-24 | `python -m deploy.installer`：git_install→alas_kill→pip_install→app_update→adb_install（Windows 图形安装器的实际动作） |
...
- **有没有官方"纯调度器"支持**：只有 `python alas.py` + `deploy/headless/requirements.txt`（无 pywebio/无 GUI 依赖的 headless 依赖清单）+ linux 模板把 `RequirementsFile` 指到它（`config_updater.py:490-499`）。没有 CLI 参数化的无头入口，也没有 systemd/service 示例；多实例无头必须自己写一行 `-c`。
- **Python 版本断言**：**全仓没有版本检查/断言**。相关痕迹只有：
  ...
  - `deploy/alas.py:43`：`sys.version_info` 只用于 win32com gen_py 目录名。
- **m0 补丁与版本/依赖差异的对应**（`m0-archive/termux/patches/` + `apply_patches.sh`、`setup_env.sh`）：
  ...
  - **抹平依赖差异**：`m0-archive/termux/shims/jellyfish.py`（jellyfish 现代版 Rust/maturin，Termux 装不了；ALAS 只用 `levenshtein_distance`，见 `module/commission/project.py`）、`setup_env.sh`（native 层走 `pkg` 的 numpy/scipy/opencv/PIL/lxml，pip 只装纯 Python 且 `pydantic<2`，`apt-mark hold python-scipy`）。
  - **与版本无关、属桥接/环境**：`module/device/method/maaal.py`+`utils.py`+`minitouch.py`+`connection.py`/`screenshot.py`/`control.py`/`app_control.py`（maaal 设备通道）、`module/ocr/rpc.py`（OCR 走 TCP 代理替换 zerorpc）、`module/base/base.py`（桥接模式预热不 import cnocr）、`module/config/argument/{argument.yaml,args.json}`（新增 Screenshot/ControlMethod 选项）、`patches/assets/*.png` + `assets_fix.py`（真机素材校准）。
...
EnableReload: false          # gui.py:89（避免派生子进程 + 触发 config/reloadalas 机制）
...
```python
# cwd 必须 = ALAS 根；sys.path 含 ALAS 根
from module.logger import logger      # 副作用：os.chdir(ALAS 根) + 建 ./log/{date}_{argv0}.txt
                                      #   logger.py:165,168,171-177,337
from module.config.config import AzurLaneConfig   # 需要 pywebio 可 import：config.py:6,742-743
from alas import AzurLaneAutoScript   # alas.py:18

alas = AzurLaneAutoScript(config_name=NAME)   # alas.py:21-28
alas.loop()                                   # 阻塞；alas.py:549-618（内部再 set_file_logger(NAME)）

# 优雅停：启动前 al 注入（同一进程内）
import threading; ev = threading.Event()
AzurLaneAutoScript.stop_event = ev            # alas.py:19,481-485,555-559；ev.set() 后 loop 退出
```
...
- **建议把 A 放进一个极薄的 `runner.py` 子进程**（`start_new_session=True`），由 wrapper 管进程组——这样 wrapper 本体**完全不 import ALAS**，只有 runner 依赖上游内部 API。

### 漂移风险点（上游最可能改）
| 风险 | 位置 | 等级 |
|---|---|---|
| `AzurLaneAutoScript.__init__/loop()/run()/stop_event`、`exit(1)` 行为 | alas.py:18-28、65-133、466-618 | 中（v3 主线，改动等于换架构） |
...
### 兜底方案（完全不 import ALAS）：可行，所需信息如下
- **准确命令行**（均需 `cwd = ALAS 根`，Python 需能 import pywebio）：
  - 默认实例：`python alas.py`（alas.py:621-623 → `AzurLaneAutoScript(config_name='alas').loop()`）；
  - 任意实例名：`python -c "from alas import AzurLaneAutoScript as A; A('alas2').loop()"`（等价 WebUI 的 `run_process` alas 分支，process_manager.py:161-166）；日志名由 `loop()` 内的 `set_file_logger(self.config_name)` 决定（alas.py:550）。
- **进程管理**：`start_new_session=True` 建进程组，停止用 `os.killpg(SIGKILL)`（ALAS **没有 SIGTERM handler**，`ProcessManager.stop()` 本身就是 `kill()`，process_manager.py:74）；退出码语义：`loop()` 正常 break/crash 均有 `exit(1)` 分支（alas.py:35-40、104、116、124、133、607）。
- **状态**：pid 存活 + 退出码 + `./config/{name}.json` 的 `{Task}.Scheduler.NextRun`；日志尾部文本兜底（"Start scheduler loop:" alas.py:551、"Scheduler: Start task `X`" alas.py:583、"exited. Reason: Finish/Update" process_manager.py:181 / alas.py:484）。
- **配置**：`os.replace` 原子写 `./config/{name}.json`（schema/锁定任务见 Q4），停着改最安全。
- 需要预先准备的信息只有三条，且都已确认：① ALAS 根路径必须作 cwd（所有路径相对）；② 需 pywebio 可 import（仅库，不需服务）；③ 无头只能跑调度器，单任务/Daemon 只能走 Q4 的 `run(task)` 或改 `Scheduler.Command`。
