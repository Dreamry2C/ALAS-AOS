...
> 代码基线：v0.1.3 之后的日志区改造（2026-09-21 落地），仓库根 `D:\VSCodeCache\maa-alas`。
...
ALAS-AOS：Android App（Kotlin + Compose），在 proot 环境里跑 ALAS（碧蓝航线自动化脚本，Python）。
三层进程模型：

```
App 进程 (Kotlin, appId=io.github.shinarin.alasaos)
...
- App 源码：`app/app/src/main/java/com/aliothmoon/maafw/`（**Java 包名刻意保留 `com.aliothmoon.maafw`，与 appId 解耦，勿"顺手"改**）。
- wrapper.py 及 ALAS 补丁：`app/app/src/main/assets/alas/{overlay,patches}/`，与仓库根 `rootfs/` 目录**双源同步**（改一边必须 cp 到另一边并 cmp 验证，铁律）。
- 启动器日志文件全部落在**外部私有目录** `/sdcard/Android/data/io.github.shinarin.alasaos/files/`（选择原因：`constant/AppFiles.kt:6-11` 注释——特权进程是 shell 身份，只有这里写得进且与 app 看到同一份；release 无 run-as，adb shell 也可读）。
- ALAS 日志落在**内部存储** `filesDir/rootfs/opt/alas/log/`——App 进程直接可读，**adb 读不到**，查看/导出必须在 App 进程内做（不经过 wrapper HTTP：wrapper 的 /logs 只服务 mtime 最新的一个 txt，历史拿不到）。
...
| 行 | 实现 | 行为 |
|---|---|---|
| 「启动器日志」 | MaaNavigationRow | 导航 `Routes.APP_LOG` → `AppLogScreen`（§3） |
| 「ALAS日志」 | MaaNavigationRow | 导航 `Routes.ALAS_LOG` → `AlasLogScreen`（§4） |
| 「导出ALAS日志」 | MaaNavigationRow | `AppRoot` 置 `exportKind=ALAS`，弹 `LogExportController` 底部 sheet（§6） |
| 「导出启动器日志」 | MaaNavigationRow | `exportKind=LAUNCHER`，同一 sheet |
| 「自动清理日志」开关 | MaaLabeledControlRow + MaaSwitch | 开直接落盘；**关弹确认框**（警告"长期使用可能占大量手机空间"）→ `SettingsIntent.SetAutoCleanLogs`（§7） |

`AppRoot` 的 sheet 状态是 `var exportKind by remember { mutableStateOf<LogExportKind?>(null) }`（可空即显隐 + 类型二合一）。

## 2. 日志文件清单（设备端真实产物）

启动器侧根：`/sdcard/Android/data/io.github.shinarin.alasaos/files/`，路径常量 `constant/AppPaths.kt` + `constant/AppFiles.kt`。
...
| `log/crash/crash_yyyyMMdd_HHmmss.txt` | `log/CrashHandler.kt`（install 于 `MaaFwApp.kt`） | 保留 10 份 |
...
ALAS 侧（proot 内 `/opt/alas/log`，实体 `filesDir/rootfs/opt/alas/log/`，访问口 `log/AlasLogSource.kt`）：
...
- 列表页 `ui/logs/AlasLogScreen.kt` + `log/AlasLogViewModel.kt`：两个分区（LazyColumn 普通 header item）——
  - 「错误记录」= `error/<毫秒时间戳>/`（纯数字名校验），ts 倒序，显示格式化时间 + 内含文件数；
  - 「按天日志」= `*.txt`，mtime 倒序，显示文件名/大小/mtime。
- txt 详情 `ui/logs/AlasLogDetailScreen.kt`：`AlasLogSource.dailyFile(name)` 解析（拒路径段）→ 尾部加载查看器。
- 错误现场详情 `ui/logs/AlasErrorDetailScreen.kt` + `log/AlasErrorDetailViewModel.kt`：log.txt 用同款尾部加载查看器（`LogTailContent`，weight 1f）+ 下方 PNG 缩略图 LazyRow（inSampleSize 采样解码，`rememberDecodedBitmap`）；点缩略图全屏 Dialog 查看（fit-center，点按关闭，**无捏合缩放**——规格明确不要）。
...
- 级别上色 `ui/logs/LogLineColors.kt`，兼容两种格式：
  - app.log `] E `/`] W `（单字母；E/A→error，W→warning；不照抄 MaaMeow 全词判据）；
  - ALAS `YYYY-MM-DD HH:MM:SS.mmm | LEVEL | msg` 的 `| ERROR |`/`| WARNING |`。
...
核心：`log/LogExportService.kt` + `LogExportKind { ALAS, LAUNCHER }`（`di/LogModule.kt` 注入 `baseDir=AppPaths.ROOT`、`launcherRoots=[LOG_DIR, DEBUG_DIR]`、`alasLogDir=AlasLogSource.logDir()`）：

| 类型 | 产物名 | 收集 | zip 条目 | properties.txt |
|---|---|---|---|---|
| ALAS | `alas_logs_yyyyMMdd_HHmmss.zip` | `LogExportCollector.collectAlas()`：txt 按文件名 `yyyy-MM-dd` 前缀、error 文件夹按毫秒 ts，**只收近 7 天**；前缀对不上的 txt 保留（多带无伤，漏现场误事） | 镜像官方 issue 格式 `log/xxx`（`log/` + 相对 ALAS 日志目录路径） | **不附** |
...
- 旧 zip 清理按类型前缀各自进行；启动器导出连带清 v0.1.3 前的 `maafw_logs_*` 遗留命名。
...
- **ALAS 部分**：`log/LogCleaner.kt`——每次冷启动在 `MaaFwApp` 的 `settings.loaded` 门控协程内（IO dispatcher）静默执行，`runCatching` 包裹，完成后 `Timber.w` 一行汇总（删了几项/释放多少）。判定基准 = 文件日期 < 今天-7 天：txt 按文件名前缀，error 文件夹按毫秒 ts（日粒度对齐）。
...
- 导出 zip 旧名 `maafw_logs_*` → 现按类型 `alas_logs_*` / `launcher_logs_*`（分享 subject 串不变）。
...
4. **ALAS 日志 adb 不可读**：内部存储，release 无 run-as；取证走 App 内导出（§6 ALAS zip）或 debug 包 run-as。
5. **wrapper /logs 只服务 mtime 最新的一个 txt**：历史 txt 与 error 现场经 HTTP 拿不到，这是 ALAS 日志页直读文件系统的原因。
6. App 日志取证（release 无 run-as，adb shell 直接可读）：
   `adb shell "tail -N /sdcard/Android/data/io.github.shinarin.alasaos/files/log/proot/session.log"`
   LogCleaner 留痕：`adb shell "grep LogCleaner /sdcard/Android/data/io.github.shinarin.alasaos/files/log/app.log"`
7. wrapper 日志取证：`adb forward tcp:22400 tcp:22400` → `curl "127.0.0.1:22400/logs?tail=200"`
8. 构建：`cd app && JAVA_HOME='D:\VSCodeCache\shizku-m\build-env\jdk-17.0.2' GRADLE_USER_HOME='D:\VSCodeCache\maa-alas\.tmp\gradle-home' cmd //c 'gradlew.bat assembleRelease --console=plain'`
9. 账册规则：完成可验证步骤后更新 `devlog.md`（倒序，最新在上）；红线：不改 ALAS 上游源码（`assets/alas/patches/` 之外）、不私自锁屏测验、push/发版需逐次授权。
