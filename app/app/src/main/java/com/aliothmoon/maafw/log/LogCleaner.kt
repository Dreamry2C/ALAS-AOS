...
/**
 * 冷启动自动清理：7 天前的 ALAS 日志静默删除（session.log 截尾在 `ProotHost.cleanupStale`）
 *
 ...
 */
class LogCleaner(
    private val alasLogSource: AlasLogSource,
) {
    ...
    fun cleanOutdated() {
        ...
        alasLogSource.dailyLogs().forEach { file ->
        ...
        alasLogSource.errorDirs().forEach { dir ->
