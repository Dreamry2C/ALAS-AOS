 ...
 *   不照抄 MaaMeow 的 `contains("[ERROR]")`：它的写入器打全词，我们打单字母
 * - ALAS 按天日志：`YYYY-MM-DD HH:MM:SS.mmm | LEVEL | msg`——全词级别
 *
...
internal fun logLineColor(line: String, error: Color, warning: Color): Color {
    ...
    return when {
        line.contains(ALAS_ERROR_MARKER) -> error
        line.contains(ALAS_WARNING_MARKER) -> warning
...
private const val ALAS_ERROR_MARKER = "| ERROR |"
private const val ALAS_WARNING_MARKER = "| WARNING |"
