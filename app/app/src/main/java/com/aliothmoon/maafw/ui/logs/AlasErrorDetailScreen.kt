...
/**
 * 一个 ALAS 错误现场的详情（二级页面）：log.txt 用共用的尾部加载查看器，
 * 下方一排 PNG 截图缩略图，点开全屏（fit-center，不做捏合缩放）
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AlasErrorDetailScreen(
    dirName: String,
    onBack: () -> Unit,
    viewModel: AlasErrorDetailViewModel = koinViewModel(),
) {
    ...
    Scaffold(
        ...
        Column(
            ...
            when {
                state.loading -> AlasErrorMessage(R.string.common_loading, Modifier.weight(1f))
                state.missing -> AlasErrorMessage(R.string.log_tail_missing, Modifier.weight(1f))
                state.logTxt == null && state.images.isEmpty() ->
                    AlasErrorMessage(R.string.log_tail_empty, Modifier.weight(1f))
                    ...
                    if (state.logTxt != null) {
                        ...
                        // 现场只剩截图：文案占位，图在下面
                        AlasErrorMessage(R.string.log_tail_missing, Modifier.weight(1f))
                    }
    ...
    fullscreenImage?.let { image ->
        AlasErrorFullscreen(image = image, onDismiss = { fullscreenImage = null })
    }
}

@Composable
private fun AlasErrorMessage(textRes: Int, modifier: Modifier = Modifier) {
    Box(
...
@Composable
private fun AlasErrorThumbnail(file: File, onClick: () -> Unit) {
...
@Composable
private fun AlasErrorFullscreen(image: File, onDismiss: () -> Unit) {
    Dialog(
        ...
        Box(
            modifier = Modifier
                ...
                .maaClickable(onClick = onDismiss),
...
private fun rememberDecodedBitmap(file: File, maxDimension: Int) = produceState<ImageBitmap?>(
    ...
    value = withContext(MaaDispatchers.IO) {
        runCatching {
