...
- **断点 1**：手机实拍帧（整帧 1280x720，与上游同约定）自制 `CLEAR_MODE_TITLE.png`/`AUTO_SEARCH_TITLE.png`，双源部署 `rootfs/patches/assets/cn/handler/` + `app/app/src/main/assets/alas/patches/assets/cn/handler/`（cmp 一致），重打包 release APK 装机，重启 App 后 overlay 幂等铺 /opt/alas。离线 sim=1.0（新）vs 0.829/0.783（旧，确定性失败）。
...
- VD 探测：`adb shell "dumpsys display | grep -oE 'displayId=[0-9]+, uniqueId=.virtual:com.android.shell,2000,MaaFw'"`（本 session 16→18 变过一次）；起游戏 `bridge_cli.py shell "am start --display <VID> -n com.bilibili.azurlane/com.manjuu.azurlane.MainActivity"`。
- overlay 机制：改 `assets/alas/patches|overlay` 只需重装 APK 重启 App，不必重烘 rootfs。
- 桌面 ALAS OCR 对质（只读合规用法）：`cd /c/other/AzurLaneAutoScript && ./toolkit/python.exe -c "from module.ocr.ocr import Ocr; ..."`（azur_lane cnocr 模型，charset 39 无 'O'）。
